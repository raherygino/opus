...
@Composable
fun RassemblementScreen(
    onEntryClick: (Int) -> Unit,
    onCreate: () -> Unit,
    viewModel: RassemblementViewModel = hiltViewModel()
) {
    ...
    Scaffold(
        ...
        floatingActionButton = {
            if (state.canCreate) {
                FloatingActionButton(
                    ...
                    Icon(Icons.Outlined.Add, contentDescription = "Nouveau rassemblement")
                }
            }
        }
    ) { padding ->
        Column(
            ...
            if (state.isLoading) {
                ...
                EmptyState("Aucun rassemblement journalier trouvé")
            } else {
                LazyColumn(
    ...
    OpusDialog(
        ...
        message = "Voulez-vous vraiment supprimer ce rassemblement journalier ?",
...
@Composable
private fun RassemblementListItem(
    entry: RassemblementJournalier,
    canDelete: Boolean,
    onClick: () -> Unit,
    onDelete: () -> Unit
) {
    Card(
        ...
        Column(modifier = Modifier.padding(14.dp)) {
            ...
            Text(
                text = "Rassemblement du ${formatDateDisplay(entry.dateRassemblement)} à ${entry.heureDisplay}",
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant
            )
            Text(
                text = "Prise d'arme : ${entry.present}/${entry.effectifTheorique} présents, ${entry.absent} absent(s)",
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant
            )
