...
fun EvenementScreen(
    ...
    onCreate: () -> Unit,
    viewModel: EvenementViewModel = hiltViewModel()
) {
    ...
    Scaffold(
        ...
        floatingActionButton = {
            if (state.canCreate) {
                FloatingActionButton(
                    ...
                    Icon(Icons.Outlined.Add, contentDescription = "Nouvel évènement")
                }
            }
        }
    ) { padding ->
        Column(
            ...
            if (state.isLoading) {
                ...
                EmptyState("Aucun évènement survenu trouvé")
            } else {
                LazyColumn(
    ...
    OpusDialog(
        ...
        message = "Voulez-vous vraiment supprimer cet évènement survenu ?",
...
@Composable
private fun EvenementListItem(
    entry: EvenementSurvenu,
    canDelete: Boolean,
    onClick: () -> Unit,
    onDelete: () -> Unit
) {
    Card(
        ...
        Column(modifier = Modifier.padding(14.dp)) {
            ...
            Text(
                text = "Le ${formatDateDisplay(entry.dateEvenement)} à ${entry.heureDisplay} — ${entry.lieuExact}",
