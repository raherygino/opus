...
@Composable
fun ObjetTrouveFormScreen(
    onSaved: () -> Unit,
    onBack: () -> Unit,
    viewModel: ObjetTrouveFormViewModel = hiltViewModel()
) {
    ...
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                title = {
                    Text(
                        if (state.isEdit) "Modifier l'objet trouvé" else "Nouvel objet trouvé",
        ...
        Column(
            ...
            FormSectionCard(
                ...
                subtitle = "Informations sur l'objet trouvé"
            ) {
                OutlinedTextField(
                    value = state.affaire,
                ...
                OpusDropdown(
                    label = "Motif de découverte *",
                    options = OBJET_TROUVE_MOTIF_LABELS.values.toList(),
                    selected = OBJET_TROUVE_MOTIF_LABELS[state.motifDecouverte] ?: state.motifDecouverte,
                    onSelect = { label ->
                        OBJET_TROUVE_MOTIF_LABELS.entries.firstOrNull { it.value == label }
                            ?.let { viewModel.updateMotifDecouverte(it.key) }
                    },
                    isError = state.motifDecouverte.isBlank() && state.errorMessage != null,
                    modifier = Modifier.fillMaxWidth()
                )
                ...
                Row(
                    ...
                    Switch(
                        checked = state.restitution,
                        onCheckedChange = viewModel::updateRestitution
                    )
                }
            }
