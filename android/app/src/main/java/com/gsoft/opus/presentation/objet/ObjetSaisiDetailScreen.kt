...
@Composable
fun ObjetSaisiDetailScreen(
    onEdit: (Int) -> Unit,
    onBack: () -> Unit,
    viewModel: ObjetSaisiDetailViewModel = hiltViewModel()
) {
    val state by viewModel.state.collectAsState()
    val snackbarHostState = remember { SnackbarHostState() }
    val context = LocalContext.current
    var viewerTarget by remember { mutableStateOf<ObjetSaisiAttachment?>(null) }
    ...
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                title = { Text("Détail — Objet saisi", fontWeight = FontWeight.Bold) },
                navigationIcon = {
        ...
        if (entry == null) {
            Column(modifier = Modifier.fillMaxSize().padding(padding).padding(16.dp)) {
                ErrorMessage(message = state.errorMessage ?: "Objet saisi introuvable")
            }
            return@Scaffold
        }
        ...
        Column(
            ...
            FormSectionCard(
                ...
                DetailRow("Type d'objet", entry.typeObjetLabel)
                if (!entry.proprietaire.isNullOrBlank()) {
                    DetailRow("Propriétaire", entry.proprietaire)
                }
                ...
                if (state.attachments.isEmpty()) {
                    ...
                    state.attachments.forEach { att ->
                        ObjetAttachmentRow(
                            ...
                            onPreview = {
                                if (isObjetImageAttachment(att.mimeType, att.originalFilename)) {
                                    viewerTarget = att
                                } else {
                                    openUrl(context, objetSaisiAttachmentDownloadUrl(entry.id, att.id))
                                }
                            },
                            onDownload = {
                                openUrl(context, objetSaisiAttachmentDownloadUrl(entry.id, att.id))
    ...
    ImageViewerDialog(
        imageUrl = viewerTarget?.let { objetSaisiAttachmentDownloadUrl(state.entry?.id ?: 0, it.id) } ?: "",
