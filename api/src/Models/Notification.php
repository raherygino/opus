...
class Notification
{
    ...
    public static function getForUser(int $userId, ?string $roleCode = null, ?string $service = null): array
    {
        ...
        if ($roleCode === 'SUPER_ADMIN' || $roleCode === 'STATION_ADMIN') {
            // Admins see all notifications — except those generated by
            // their own actions.
            $sql = 'SELECT n.*, p.im AS personnel_im, p.lastname AS personnel_nom,
                           p.firstname AS personnel_prenoms, p.grade AS personnel_grade,
                           p.photo AS personnel_photo,
                    ...
                    LEFT JOIN users cu ON n.created_by = cu.id
                    LEFT JOIN personnel cp ON cu.personnel_id = cp.id
                    WHERE n.created_by IS NULL OR n.created_by <> ?
            ...
            $sql = 'SELECT n.*, p.im AS personnel_im, p.lastname AS personnel_nom,
                    ...
                    ORDER BY n.created_at DESC';
            $stmt = $db->prepare($sql);
            $stmt->execute([$userId, $service, $service, $userId]);
        }
        return $stmt->fetchAll();
    }
    ...
    public static function getUnreadCount(int $userId, ?string $roleCode = null, ?string $service = null): int
    {
        ...
        if ($roleCode === 'SUPER_ADMIN' || $roleCode === 'STATION_ADMIN') {
            $sql = 'SELECT COUNT(*) FROM notifications WHERE is_read = 0 AND (created_by IS NULL OR created_by <> ?)';
            $stmt = $db->prepare($sql);
            $stmt->execute([$userId]);
        } else {
            $sql = 'SELECT COUNT(*) FROM notifications WHERE is_read = 0 AND (user_id = ? OR (user_id IS NULL AND (? IS NULL OR service = ?))) AND (created_by IS NULL OR created_by <> ?)';
            $stmt = $db->prepare($sql);
            $stmt->execute([$userId, $service, $service, $userId]);
        }
    ...
    private static function sendPush(int $id, array $data): void
    {
        try {
            ...
            if (!empty($data['user_id'])) {
                ...
                // Broadcast push: deliver to admins EXCEPT the actor who
                // triggered the notification — a user must never receive a
                // push for their own action.
                $exclude = !empty($data['created_by']) ? [(int) $data['created_by']] : [];
                \App\Helpers\FcmSender::sendToAdmins($payload, $exclude);
            }
        } catch (\Throwable $e) {
