...
/**
 * OPUS — Seed arme + type_arme catalogue
 *
 * Applies ONLY database/035_seed_arme_type_arme.sql to the dev DB.
 * Non-destructive: does NOT drop/recreate the database. The seed file
 * itself uses INSERT ... ON DUPLICATE KEY UPDATE so it is re-runnable.
 *
 * Usage (from the api/ folder):
 *   php seed-arme.php
 *
 * Or with custom DB credentials via env:
 *   DB_HOST=127.0.0.1 DB_NAME=opus DB_USER=root DB_PASS= php seed-arme.php
 */
...
$seedFile = realpath(__DIR__ . '/../database/035_seed_arme_type_arme.sql');
if (!$seedFile || !is_readable($seedFile)) {
    fwrite(STDERR, "❌ Seed file not found: database/035_seed_arme_type_arme.sql\n");
    exit(1);
}

echo "🔧 OPUS — Seed arme + type_arme\n";
echo "===============================\n\n";
...
// --- Pre-flight: confirm the target tables exist ---
foreach (['type_arme', 'arme'] as $table) {
    try {
        $pdo->query("SELECT 1 FROM `$table` LIMIT 1");
    } catch (PDOException $e) {
        fwrite(STDERR, "❌ Table `$table` is missing — run migrations 030/031/034 first (or api/reset-db.php).\n");
        exit(1);
    }
}
echo "✅ Tables `type_arme` and `arme` present\n\n";
...
// --- Report row counts ---
$typeCount = (int) $pdo->query("SELECT COUNT(*) FROM `type_arme`")->fetchColumn();
$armeCount = (int) $pdo->query("SELECT COUNT(*) FROM `arme`")->fetchColumn();

echo "✅ Seed applied successfully\n";
echo "   • type_arme rows: $typeCount\n";
echo "   • arme rows:      $armeCount\n";
echo "\n===============================\n";
echo "✅ Done\n";
